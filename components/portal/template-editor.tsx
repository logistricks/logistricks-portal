...
export function TemplateEditor({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-4">
      <div className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl duration-200 animate-in fade-in zoom-in-95 sm:rounded-2xl">
        ...
        <div className="grid flex-1 grid-cols-1 overflow-hidden lg:grid-cols-3">
          ...
          <div className="flex flex-col overflow-y-auto p-5 lg:col-span-2">
            <input
              ...
              className="mb-4 w-full border-0 border-b border-[#E2E8F0] pb-2 text-lg font-bold text-[#0D1B2A] outline-none placeholder:text-[#94A3B8] focus:border-[#F97316]"
            />

            {isEmail && (
              <div className="mb-3 flex items-center gap-2 rounded-md border border-[#E2E8F0] px-3">
                <span className="text-sm font-medium text-[#64748B]">Subject:</span>
                <input
                  value={form.subject ?? ""}
                  onChange={(e) => setForm((f) => ({ ...f, subject: e.target.value }))}
                  placeholder="Email subject line"
                  className="h-10 flex-1 border-0 text-sm outline-none placeholder:text-[#94A3B8]"
                />
              </div>
            )}

            {/* Toolbar */}
            <div className="flex flex-wrap items-center gap-1 rounded-t-md border border-[#E2E8F0] bg-[#F8FAFC] p-1.5">
              {(isEmail ? emailTools : waTools).map((T, i) =>
                T === "|" ? (
                  <span key={i} className="mx-1 h-5 w-px bg-[#E2E8F0]" />
                ) : (
                  <button
                    key={i}
                    type="button"
                    className="rounded p-1.5 text-[#64748B] transition-colors hover:bg-white hover:text-[#0D1B2A]"
                  >
                    {(() => {
            ...
            <textarea
              ...
              className="min-h-64 flex-1 resize-none rounded-b-md border border-t-0 border-[#E2E8F0] p-4 text-sm leading-relaxed text-[#0F172A] outline-none focus:border-[#F97316]"
            />
            ...
            <div className="mt-4">
              ...
              <div className="rounded-md border border-[#E2E8F0] bg-white p-3 text-sm leading-relaxed text-[#0F172A]">
          ...
          {/* Variable panel */}
          <div className="flex flex-col overflow-y-auto border-t border-[#E2E8F0] bg-[#F8FAFC] p-4 lg:border-l lg:border-t-0">
                ...
                return (
                  <div key={group.group}>
                    <button
                      ...
                      className="flex w-full items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-[#0D1B2A]"
                    >
                      ...
                      <div className="mt-2 space-y-1">
                        {group.vars.map(([name, label]) => (
                          <button
                            ...
                            className="flex w-full items-center justify-between rounded border border-transparent px-3 py-2 text-left transition-colors hover:border-[#E2E8F0] hover:bg-white"
                          >
        ...
        {/* Footer */}
        <div className="flex flex-col gap-4 border-t border-[#E2E8F0] p-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
                  ...
                  <button
                    ...
                    className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${
                      on
                        ...
                        : "border-[#E2E8F0] bg-white text-[#64748B] hover:border-[#F97316]/40"
                    }`}
                  >
                    {c.name}
                  </button>
            ...
            <label className="mt-3 flex items-center gap-2 text-sm text-[#0F172A]">
              <input
                type="checkbox"
...
function renderPills(body: string) {
  ...
  return parts.map((p, i) =>
    /^\{\{\w+\}\}$/.test(p) ? (
      <span
        key={i}
        className="mx-0.5 inline-flex rounded bg-[#FFF7ED] px-1.5 py-0.5 align-baseline text-xs font-medium text-[#F97316]"
      >
        {p}
      </span>
    ) : (
