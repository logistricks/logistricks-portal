...
export async function PATCH(req: NextRequest) {
  ...
  // is_complete_reply_template: only one per client — unset all others first
  if (body.is_complete_reply_template !== undefined) {
    const db = admin()
    if (body.is_complete_reply_template) {
      await db
        .from("templates")
        .update({ is_complete_reply_template: false })
        .eq("client_code", session.clientCode)
        .eq("is_complete_reply_template", true)
      const { error } = await db
        .from("templates")
        .update({ is_complete_reply_template: true, is_reply_template: false, is_missing_reply_template: false })
        .eq("client_code", session.clientCode)
        .eq("template_id", body.template_id)
      if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    } else {
