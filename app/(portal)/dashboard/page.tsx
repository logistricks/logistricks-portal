        ...
        {placeholders.map((d, i) => (
          <div key={d} className="group relative flex flex-1 flex-col items-center gap-1.5">
            <div className="w-full flex-1 flex flex-col justify-end">
              <div
                className="w-full rounded-t animate-pulse bg-[#E2E8F0] dark:bg-[#1E3A5F]"
                style={{ height: `${20 + (i * 12) % 70}%` }}
              />
            </div>
            <span className="text-[10px] font-semibold text-[#94A3B8] dark:text-[#475569]">{d}</span>
          </div>
        ))}
      </div>
    )
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="rounded border border-[#E2E8F0] bg-white shadow-[0_1px_4px_rgba(0,0,0,0.06)] dark:border-[#1E3A5F] dark:bg-[#111E33] lg:col-span-3">
          <div className="flex items-center justify-between border-b border-[#E2E8F0] px-5 py-4 dark:border-[#1E3A5F]">
            <div>
              ...
              <p className="text-xs text-[#64748B] dark:text-[#475569]">Requests received — last 7 days</p>
            </div>
            {!loading && (
      ...
      {/* Recent Requests */}
      <div className="rounded border border-[#E2E8F0] bg-white shadow-[0_1px_4px_rgba(0,0,0,0.06)] dark:border-[#1E3A5F] dark:bg-[#111E33]">
        <div className="flex items-center justify-between border-b border-[#E2E8F0] px-5 py-4 dark:border-[#1E3A5F]">
          <div>
        ...
        {loading ? (
          <div className="divide-y divide-[#F1F5F9] dark:divide-[#1A2A40]">
            {[1,2,3,4,5].map((i) => (
          ...
          <ul className="divide-y divide-[#F1F5F9] dark:divide-[#1A2A40]">
            {recent.length === 0 ? (
              <li className="px-5 py-8 text-center text-sm text-[#94A3B8]">No requests yet.</li>
            ) : (
              recent.map((r) => (
                <li key={r.id}>
                  <Link
